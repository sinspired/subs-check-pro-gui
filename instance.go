...
//
// 机制：
//  1. 尝试监听固定本地 IPC 端口（127.0.0.1:29741）。
//     - 成功 → 当前是首个实例，后台持续监听唤醒信号。
//     - 失败 → 向已有实例发送 SHOW 握手包，然后退出本进程。
//  2. 发送 SHOW 失败（端口被无关程序占用）→ 优雅降级，允许多实例。
//
// 注意：ensureSingleInstance 必须在 main() 最开始、Wails 初始化之前调用。
package main
